...
def pre_fork(server, worker):
    try:
        # 根据每个线程用的卡数算
        # gid1, gid2 = gpuDevicePool.pop(0), gpuDevicePool.pop(0)
        gid = gpuDevicePool.pop(0)
    except:
        # gid1 = (worker.age - 1) % gpuDeviceCount
        # gid2 = (worker.age) % gpuDeviceCount
        
        gid = (worker.age - 1) % gpuDeviceCount
    worker.gid = gid
    # worker.gid = (gid1, gid2)
 
def post_fork(server,worker):
    # os.environ['CUDA_VISIBLE_DEVICES'] = f'{worker.gid[0]},{worker.gid[1]}'
    # server.log.info(f'worker(age:{worker.age}, pid:{worker.pid}, cuda:{worker.gid1},{worker.gid2})')
...
bind = '0.0.0.0:63000'  # 绑定监听ip和端口号和启动docker端口映射一致
