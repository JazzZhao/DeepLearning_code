...
feasibility_report_prompt = """你现在是一个从事技术文档编写的技术人员，现在需要撰写\"{}\",
            主题如下：\n\"{}\"，主题到此结束。 
            参考材料如下：\n\"{}\"，参考材料到此结束。
            输出格式需要为以下格式，例如： 
            \n\"{}\"
            注意：生成内容必须只包含示例中的序号，生成内容的序号必须严格按照以上示例的格式进行生成，生成的内容不能直接引用以上材料的内容！生成的内容必须根据参考材料进行二次创作！
"""

all_feasibility_report_prompt = """你现在是一个从事技术文档编写的技术人员，你需要结合参考材料，按照主题的要求生成专业技术的文档， 
            主题如下：\n\"{}\"，主题到此结束。 
            参考材料如下：\n\"{}\"，参考材料到此结束。 
            注意：生成的内容不能直接引用以上材料的内容！生成的内容必须根据参考材料进行二次创作！生成的内容必须具体专业性！
"""
...
dir_feasibility_report_prompt = """你现在是一个从事技术文档编写的技术人员， 你需要根据下面的材料攥写大纲， 
            ...
            三、
                3.1
                3.2
            注意：生成内容的序号必须严格按照以上示例的格式进行生成，生成的内容不能直接引用以上材料的内容！生成的内容必须根据参考材料进行二次创作！
"""
...
# 获取周报prompt
def get_feasibility_report_prompt(query, knowledge, mode="dir", section = ""):
    """
        获取对应模板不通占位符的prompt
